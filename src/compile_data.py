...
def calculate_fractions(dfs):    
    ...
    df_progenitors_glial = dfs[0][['condition', 'day', 'bio_rep', 'tech_rep']].copy()

    # Progenitors = (SOX10-positive – SOX10/S100b double-positive) / total cells
    df_progenitors_glial['progenitors'] = (
        (dfs[0]['total_sox10'] - dfs[0]['total_s100b']) / dfs[0]['total_cells']
    )

    # Glial = SOX10/S100b double-positive / total cells
    df_progenitors_glial['glial'] = dfs[0]['total_s100b'] / dfs[0]['total_cells']

    # Neural fraction = HuCD-positive / total cells
    df_neural = dfs[1][['condition', 'day', 'bio_rep', 'tech_rep']].copy()
    df_neural['neural'] = dfs[1]['total_hucd'] / dfs[1]['total_cells']
